"""
Improved MuJoCo Benchmark Tests for DHC-SSM Architecture.

Uses the improved RL adapter with better architecture for handling
various observation dimensions.
"""
...
def test_mujoco_environment_compatibility():
    """Test compatibility with various MuJoCo environments using improved adapter."""
    print("\n=== Test 1: MuJoCo Environment Compatibility (Improved) ===")
    ...
    for env_name in environments:
        try:
            ...
            # Create policy with improved adapter
            config = DHCSSMConfig()
...
def test_policy_rollout():
    ...
    print("\n=== Test 2: Policy Rollout (Improved) ===")
...
def test_training_on_mujoco():
    ...
    print("\n=== Test 3: Learning from MuJoCo Data (Improved) ===")
...
def test_performance_metrics():
    ...
    print("\n=== Test 4: Performance Metrics (Improved) ===")
...
def test_multi_environment_benchmark():
    ...
    print("\n=== Test 5: Multi-Environment Benchmark (Improved) ===")
    
    environments = ["Pendulum-v1", "HalfCheetah-v5", "Hopper-v5", "Walker2d-v5"]
    results = {}
...
def test_actor_critic_adapter():
    """Test the actor-critic adapter."""
    print("\n=== Test 6: Actor-Critic Adapter ===")
    ...
    try:
        ...
        with torch.no_grad():
            actions, values = actor_critic(obs_tensor)
        
        assert actions.shape == (1, action_dim), "Action shape mismatch"
        assert values.shape == (1, 1), "Value shape mismatch"
        
        env.close()
        
        print(f"✓ Actor-Critic adapter test passed")
...
def run_all_mujoco_tests():
    """Run all improved MuJoCo benchmark tests."""
    print("=" * 70)
    print("DHC-SSM v3.1 - IMPROVED MUJOCO BENCHMARK")
    print("=" * 70)
    ...
    tests = [
        ...
        ("Actor-Critic Adapter", test_actor_critic_adapter),
    ]
    ...
    print("\n" + "=" * 70)
    print("IMPROVED MUJOCO BENCHMARK RESULTS")
    print("=" * 70)
    ...
    results_file = "tests/mujoco_improved_results.json"
    try:
