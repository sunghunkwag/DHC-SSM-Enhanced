"""
Extended benchmark suite for additional MuJoCo environments and Atari.
"""
...
def run_extended_benchmarks():
    print("="*70)
    print("EXTENDED BENCHMARK SUITE")
    print("="*70)
    print()
    
    os.makedirs("benchmarks_extended", exist_ok=True)
    
    # MuJoCo benchmarks
    mujoco_envs = ["Walker2d-v4", "HalfCheetah-v4"]
    
    for env_name in mujoco_envs:
        print(f"\nBENCHMARK: {env_name} (200 episodes)")
        print("="*70)
        
        ssm_rewards = train_model(env_name, 200, use_ssm=True, seed=42)
        mlp_rewards = train_model(env_name, 200, use_ssm=False, seed=42)
        
        env_slug = env_name.lower().replace("-", "_")
        
        with open(f"benchmarks_extended/{env_slug}_200.json", "w") as f:
            json.dump({
                "ssm_rewards": [float(r) for r in ssm_rewards],
                "mlp_rewards": [float(r) for r in mlp_rewards]
            }, f, indent=2)
        
        with open(f"benchmarks_extended/{env_slug}_200.csv", "w", newline="") as f:
            writer = csv.writer(f)
    ...
    print("\n" + "="*70)
    print("ALL EXTENDED BENCHMARKS COMPLETED")
    print("="*70)
    print("\nResults saved to benchmarks_extended/ directory")
